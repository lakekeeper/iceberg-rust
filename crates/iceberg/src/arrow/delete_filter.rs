...
pub(crate) mod tests {
    ...
    async fn test_equality_delete_waiter_is_notified_after_load_started() {
        ...
        let waiter = filter.get_equality_delete_set_for_delete_file_path(delete_file_path);
        tokio::pin!(waiter);
        assert!(matches!(futures::poll!(&mut waiter), Poll::Pending));

        let (tx, rx) = tokio::sync::oneshot::channel();
        filter.insert_equality_delete(delete_file_path, rx);
        tx.send(Arc::new(EqDeleteSet {
            keys: HashSet::from([EqDeleteKey(vec![Some(Datum::long(10))])]),
            fields: vec![("id".to_string(), 1, Type::Primitive(PrimitiveType::Long))],
        }))
        .unwrap();
        ...
        let set = tokio::time::timeout(Duration::from_secs(1), waiter)
            ...
            .expect("equality delete set should be loaded");

        assert_eq!(
            set.keys,
            HashSet::from([EqDeleteKey(vec![Some(Datum::long(10))])])
        );
    }
}
